from backend.repository.artifact_versions import create_artifact_version, update_artifact_approval
from backend.repository.evidence_sources import create_evidence_source, list_evidence_sources
...
def test_upsert_model_element_updates_not_duplicates(db_session):
    ...
    second = upsert_model_element(name="Payment Service (renamed)", **{**args, "current_commit": "def456"})
...
def test_artifact_approval_is_idempotent(db_session):
    ...
    version = create_artifact_version(
        db_session, system_id=system.id, commit_sha="abc123", phase="as-is", author_type="agent"
    )
...
def test_evidence_sources_create_and_list(db_session):
    system = make_system(db_session)
    create_evidence_source(db_session, system_id=system.id, source_type="code", location="/evidence/code/")
